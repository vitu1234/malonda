...
public class UnitsAutoCompleteAdapter extends BaseAdapter implements Filterable {
    ...
    private ArrayList<Unit> originalList;
    private ArrayList<Unit> suggestions = new ArrayList<>();
    private Filter filter = new CustomFilter();

    public UnitsAutoCompleteAdapter(Context context, ArrayList<Unit> originalList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        try {
            ...
            Unit model = suggestions.get(position);

            AppCompatTextView tvTitle = convertView.findViewById(R.id.text1);
            AppCompatTextView id = convertView.findViewById(R.id.text2);
            tvTitle.setText(model.getUnit_name()+" - "+model.getUnit_symbol());
            id.setText(model.getUnit_id()+"");
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (originalList != null && constraint != null) {
                for (int i = 0; i < originalList.size(); i++) {
                    if (originalList.get(i).getUnit_name().toLowerCase().contains(constraint.toString().toLowerCase().trim())) {
